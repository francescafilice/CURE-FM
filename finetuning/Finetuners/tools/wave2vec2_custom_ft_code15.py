# Forward pass of Wav2Vec2Model from .../fairseq-signals/fairseq_signals/models/wav2vec2/wav2vec2.py
...
@dataclass
class Wav2Vec2ConfigCustomFtCode15(ECGTransformerConfig):
...
@register_model("wav2vec2custom_ft_code15", dataclass=Wav2Vec2ConfigCustomFtCode15)
class Wav2Vec2ModelCustomFtCode15(ECGTransformerModel):
    def __init__(self, cfg: Wav2Vec2ConfigCustomFtCode15):
        super().__init__(cfg)
        self.cfg = cfg
        ...
        self.classification_head = nn.Linear(cfg.encoder_embed_dim, 1)
    ...
    def forward(
        ...
        if self.feature_grad_mult > 0:
            features = self.feature_extractor(source)   # call to feature extractor
        ...
        # Transformer embedding (dim=768) goes in encoder_result
        encoder_result = self.encoder(x, padding_mask=padding_mask)

        x = encoder_result["x"]
        
        # Apply classification head for binary classification
        logits = self.classification_head(x[:, -1, :])  # Use last token for classification
